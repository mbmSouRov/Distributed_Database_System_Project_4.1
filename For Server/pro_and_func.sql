...
CREATE OR REPLACE PACKAGE BODY components AS
	...
	BEGIN
		if a=1 then
			...
			FOR R IN (select * from processor where (price > first1 AND price < last1)) LOOP
	...
	BEGIN
		if a=3 then
			...
			FOR R IN (select * from ram where (price > first2 AND price < last2)) LOOP
	...
	BEGIN
		if a=3 then
			...
			FOR R IN (select * from gpu where (price > first3 AND price < last3)) LOOP
	...
	BEGIN
		if a=3 then
			...
			FOR R IN (select * from motherboard where (price > first4 AND price < last4)) LOOP
	...
	BEGIN
		if a=3 then
			...
			FOR R IN (select * from ssd where (price > first5 AND price < last5)) LOOP
	...
	BEGIN
        FOR R IN (SELECT * FROM processor where pro_id = S1) LOOP
			a:= R.price;
			a1:= R.name;
		END LOOP;
		
		FOR R IN (SELECT * FROM ram where ram_id = S2) LOOP
			b:= R.price;
			b1:= R.name;
		END LOOP;
		
		FOR R IN (SELECT * FROM gpu where gpu_id = S3) LOOP
			c:= R.price;
			c1:= R.name;
		END LOOP;
		
		FOR R IN (SELECT * FROM motherboard where mobo_id = S4) LOOP
			d:= R.price;
			d1:= R.name;
		END LOOP;
		
		FOR R IN (SELECT * FROM ssd where ssd_id = S5) LOOP
			e:= R.price;
			e1:= R.name;
		END LOOP;
